...
import random
import numpy as np
from DVS_GESTURE.util.transforms import build_transforms
from DVS_GESTURE.util import utils, dataset
...
def load_data(distributed, args):
    print("Loading data")
    data_transform = build_transforms(args)
    dataset_path = args.data_path
    assert os.path.exists(dataset_path), "{} path does not exist.".format(dataset_path)
    train_dataset = dataset.DVS128Gesture(root=args.data_path,
                                          train=True,
                                          data_type='frame',
                                          frames_number=args.STEPS,
                                          split_by='number',
                                          transform=data_transform["train"]
                                          )
    validate_dataset = dataset.DVS128Gesture(root=args.data_path,
                                             train=False,
                                             data_type='frame',
                                             frames_number=args.STEPS,
                                             split_by='number',
